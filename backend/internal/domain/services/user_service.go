...
func (s *userService) Create(ctx context.Context, pharmacyID uuid.UUID, actorRole string, email, password, name, role string) (*models.User, error) {
	if role == "" {
		role = RoleStaff
	}
	if actorRole == RoleManager {
...
func (s *userService) Update(ctx context.Context, pharmacyID uuid.UUID, actorRole string, userID uuid.UUID, name string, role *string, isActive *bool) (*models.User, error) {
	u, err := s.userRepo.GetByID(ctx, userID)
	if err != nil || u == nil {
		return nil, errors.ErrNotFound("user")
	}
