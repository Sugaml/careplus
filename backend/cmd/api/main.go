...
func main() {
	...
	orderService := services.NewOrderService(orderRepo, productRepo, inventoryService, promoCodeRepo, promoCodeService, customerRepo, customerMembershipRepo, referralPointsServiceInterface, zapLogger)
	...
	router := http.NewRouter(cfg, authHandler, pharmacyHandler, productHandler, categoryHandler, productUnitHandler, membershipHandler, reviewHandler, orderHandler, promoCodeHandler, paymentHandler, inventoryHandler, invoiceHandler, configHandler, usersHandler, uploadHandler, activityHandler, notificationHandler, promoHandler, referralHandler, healthHandler, dutyRosterHandler, dailyLogHandler, dashboardHandler, authProviderInterface, userRepo, activityLogServiceInterface, zapLogger)
